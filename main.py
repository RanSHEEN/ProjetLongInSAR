...
model_card = MLP(file='cardioid_hidden_out.txt', hidden_sizes=[1024, 256], activation='cardioid')
...
model_z = MLP(file='z_hidden_out.txt', hidden_sizes=[1024, 256], activation='zReLU')
print('ZReLU model')
print(model_z)
...
model_mod = MLP(file='mod_hidden_out.txt', hidden_sizes=[1024, 256], activation='modReLU')
print('ModReLU model')
print(model_mod)
...
for model, optimizer, train, val, name in zip(model_list, optimizer_list, train_losses, val_losses, model_names):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in zip(in_train_dataloader, out_train_dataloader):
            ...
            loss = weighted_arg_mod_loss(outputs, targets)
            loss.backward()
            optimizer.step()
...
for i, (train_loss, val_loss, model_name) in enumerate(zip(train_losses, val_losses, ['Cardioid', 'zReLU', 'modReLU'])):
